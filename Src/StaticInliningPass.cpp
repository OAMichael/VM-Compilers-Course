...
StaticInliningPass::StaticInliningPass() : Pass(STATIC_INLINING_PASS_NAME) {}
...
void StaticInliningPass::InlineCallees(Function* func) {
    ...
    for (auto* instCall : calls) {
        ...
        if (callee->GetInstructionCount() <= INLINE_INSTRUCTION_COUNT_THRESHOLD) {
            InlineCall(instCall);
        }
    }
...
void StaticInliningPass::InlineCall(InstructionCall* instCall) const {
    ...
    // Copy callee graph to insert it to caller graph (TODO: implement callee cache)
    ...
    while (calleeEntryInst) {
        ...
        if (calleeEntryInst->IsPhi()) {
            callBB->PrependInstruction(calleeEntryInst);
        }
        else {
    ...
    callBB->SetTrueSuccessor(calleeEntry->GetTrueSuccessor());
    callBB->SetFalseSuccessor(calleeEntry->GetFalseSuccessor());
    for (auto* pred : calleeEntry->GetPredecessors()) {
        callBB->AddPredecessor(pred);
    }
    ...
    for (auto* calleeBB : callee->GetBasicBlocks()) {
        ...
        calleeBB->SetParentFunction(caller);
    }


    // Rebind all inputs if there are any
    ...
    if (calleeRets.size() == 1) {
        ...
        calleeRetBB->SetTrueSuccessor(callBBTrueSucc);
        calleeRetBB->SetFalseSuccessor(callBBFalseSucc);
    }
    else {
        BasicBlock* postCallBB = IrBuilder->CreateBasicBlock(caller, "PostCallTo_" + calleeOrig->GetName());

        // For all basic blocks of the callee which end with Ret instruction replace the instruction with Jump to post call BB and create Phi
